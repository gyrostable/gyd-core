...
contract ReserveStewardshipIncentives is IReserveStewardshipIncentives, Governable {
    ...
    uint256 internal constant OVERESTIMATION_PENALTY_FACTOR = 0.1e18;
    uint256 internal constant MAX_HEALTH_VIOLATIONS = 1; // TODO should this be configurable?
    ...
    function completeInitiative() external {
        ...
        require(
            reserveHealthViolations.nViolations <= MAX_HEALTH_VIOLATIONS,
            "initiative failed: too many health violations"
        );

        // Compute target reward
        uint256 gydSupply = gydToken.totalSupply();
        uint256 aggSupply_ = aggSupply.aggSupply +
            (initiative.endTime - aggSupply.lastUpdatedTime) *
            gydSupply;
