...
interface IReserveStewardshipIncentives {
    ...
    event InitiativeCompleted(uint256 startTime, address rewardTo, uint256 rewardGYDAmount);
    ...
    /// @notice Complete the active initiative and claim rewards. The initiative period must have passed while the reserve health conditions have held, and they must currently still hold.
    /// @param rewardsTo address where rewards (in GYD) should be trasferred. Usually a governance treasury.
    function completeInitiative(address rewardsTo) external;
