...
interface IPAMM {
    /// @notice Quotes the amount of GYD for `vaultTokenTuples`
    /// @param vaultTokenTuples a basket of vault token and associated amounts to be priced
    /// @param mintFee the fee to be charged for minting
    /// @return gydAmount the price of the input basket in GYD
    function calculateGYDToMint(DataTypes.TokenTuple[] memory vaultTokenTuples, uint256 mintFee)
        external
    ...
    function calculateAndRecordGYDToMint(
        DataTypes.TokenTuple[] memory vaultTokenTuples,
        uint256 mintFee
    ) external returns (uint256 gydAmount);

    /// @notice Quotes the amount of GYD for `vaultTokenTuples`
    /// @param vaultTokenTuples an basket of vault token and associated amounts  to be priced
    /// @param redeemFee the fee to be charged for redeeming
    /// @return gydAmount the price of the input basket in GYD
    function calculateGYDToBurn(DataTypes.TokenTuple[] memory vaultTokenTuples, uint256 redeemFee)
        external
        returns (uint256 gydAmount);
}
